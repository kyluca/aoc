...
def main():
    ...
    max_sleeping_guard = max(guard_sleeping_sums.items(), key=lambda x: x[1])

    print(max_sleeping_guard)

    max_sleeping_time = defaultdict(list)
    for log in guard_logs[max_sleeping_guard[0]]:
    ...
    print(max_sleeping_minute)
    print(int(max_sleeping_guard[0]) * max_sleeping_minute[0])
